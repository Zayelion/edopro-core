...
/**
 * Ports the native `field::process(Processors::ExecuteOperation&)` control flow.
 ...
 */
function processExecuteOperation(unit, field) {
  ...
  const result = lua?.call_coroutine
    ? lua.call_coroutine(triggeringEffect.operation, count, yieldValue, step)
    : undefined;
...
module.exports = processExecuteOperation;
