...
/**
 * Ports the native `field::process(Processors::ExecuteCost&)` control flow.
 ...
 */
function processExecuteCost(unit, field) {
  ...
  const result = lua?.call_coroutine
    ? lua.call_coroutine(triggeringEffect.cost, count, yieldValue, step)
    : undefined;
...
module.exports = processExecuteCost;
